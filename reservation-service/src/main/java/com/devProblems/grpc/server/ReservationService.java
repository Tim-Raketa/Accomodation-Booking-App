...
public class ReservationService extends ReservationServiceGrpc.ReservationServiceImplBase {
    ...
    public void deleteReservation(AccommodationId request, StreamObserver<isAvailable> responseObserver) {
        ...
        try {
            reservation = repository.findById(Long.valueOf(request.getId())).get();
        } catch (Exception e) {
    ...
    @Override
    public void hadFiveReservationsInPast(AccommodationId request, StreamObserver<FiveReservationsInPast> responseObserver) {
        List<Reservation> reservations = repository.findAll().stream()
                .filter(reservation -> reservation.getAccommodationId() == request.getId() &
                        reservation.getStatus()== ReservationStatus.RESERVATION_STATUS_ACCEPTED &
                        reservation.getEndTime().isBefore(LocalDate.now()))
